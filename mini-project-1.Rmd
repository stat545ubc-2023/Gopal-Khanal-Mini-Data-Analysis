...
```{r}
glimpse(flow_sample) # Overview of the data-of of row and columns, and col names

class(flow_sample) # class of dataset

flow_sample %>% 
 summarise((avg=mean(flow, na.rm=TRUE))) # mean value of the variable 'flow'
...
# Minimum flow of all years irrespective of extreme type

flow_sample %>%
  summarise(min.flow=min(flow, na.rm=TRUE))
...
```{r}
# Overview of the data-of of row and columns, and col names
 glimpse(vancouver_trees)

# dimesion of data
dim(vancouver_trees)

# mean diameter of all trees
vancouver_trees %>% 
 summarise((avg=mean(diameter, na.rm=TRUE)))

# Average diameter of tree presence or absence of "Curb"
vancouver_trees %>% 
group_by(curb) %>%
summarise((avg=mean(diameter, na.rm=TRUE)))
...
# Summarize mean area of the cell nuclei depending upon whether tumor was of benign or malignant

cancer_sample %>% 
  group_by(diagnosis) %>%
summarise((avg=mean(area_mean, na.rm=TRUE)))

# Max Area of worst breast tumor
cancer_sample %>% 
...
# Total number of housing permits every year
building_permits%>%
  group_by(year) %>%
summarise(n= n())
...
_I've chosen the flow_sample data because it has annual trends of flow over long period of time, and I wanted to see how that varies over time and months. And there are some missing values as well to account for._
...
**Based on eye balling of scatter plot of flow over years show that minimum flow appears to be increasing over the years whereas the maximum flow appears to be reflecting decreasing trend. But we don't have whether these trends are statistically significant. I'm therefore interested in examining following four questions in the next independent exercise.""

1. Whether there there is statistically significant trend in both extreme minimum, and extreme maximum flow (m3/sec) over the years? I'll use linear regression model to see annual trends. I'll check the significance of beta coefficient. I'll also try and explore non-parametric Mann-Kendall and Sen's methods tests to determine whether there is a positive or negative trend.

2. Compare flow pattern among decades:whether there are some decades (say 1970-1980) with unusual extreme min and max flow? Which years had more than average of extreme maximum flow? Are extreme flows (both min and max) after 2000 are different than those before 2000? If there has been consistent increase in extreme max flow after 2000, this MAY be signature of increase in extreme rainfall after 2000 or MAY be of extreme runoff in the rivers,
